...
| `output.surface` | `0` | production model uses `vol_gm`/`vol_wm`/`vol_csf` only (`config/models/stacked.yaml`) — SNBB's surface outputs were computed but are never ingested/consumed. **Must patch**: this build's shipped default is `1` |
...
- [x] Confirm feature schema needs: volume-only vs. surface. Done 2026-08-21 — production model (`config/models/stacked.yaml`) is `vol_gm`/`vol_wm`/`vol_csf` only; surface was computed for SNBB but never ingested/consumed. `output.surface = 0` confirmed correct.
...
- [ ] MCR (`container/mcr.zip`) — in transfer as of 2026-08-21, not yet staged. `%post` handles either an already-installed runtime tree or a raw MathWorks installer package.
- [ ] Write `container/atlas/PROVENANCE.md`.
...
- [ ] §6 reproducibility suite (12 stratified + 2 stress subjects, comparing against stored training features) — not yet run; the single ad-hoc smoke test above confirms the mechanism works, not that it reproduces training-time features.
