...
Non-negotiable evaluation rules baked into the harness: splits are **grouped by subject** (a subject's repeated scans never span train/test), and now stratified on age decile too (`bagpipe.models.evaluate._make_strata`, added in the Phase 2 harness rework — plain `GroupKFold` had no way to keep a small old-age tail from landing lopsidedly in one fold); the stacked model's OOF predictions for the meta-learner are generated inside the same fold structure (no leakage from base to meta), and that fold structure is itself subject-grouped (`RegionalStackingRegressor`'s own internal `outer_cv` used a plain, non-grouped split until Phase 2 fixed it — a real bug, not by design); metrics are MAE (primary) plus r/R² (reported), plus per-age-band MAE and a `cole_slope` diagnostic since the model's error is strongly age-dependent; and **age-bias correction is a pluggable step** — de Lange & Cole-style linear correction (fit `predicted_age ~ true_age` on training folds, apply to test) is the default, Beheshti's variant an alternative, selected by config. The held-out predictions the corrector fits on come from a leave-one-outer-fold-out estimator by default (`bias_correction_method="lofo"`, ~6x cheaper than the original nested-CV estimator it replaced, verified to agree with it within noise on real data) — a nested variant is kept for that verification, not routine use. Corrected and uncorrected BAG are both stored in `predictions`.
...
Flow: user uploads a T1w scan — **DICOM (zip of a series) or NIfTI directly** — on a web page → job enters a queue → worker converts DICOM to NIfTI if needed (dcm2niix) → defaces (pydeface) → runs the **same CAT12 preprocessing used in training** (CAT12 standalone build, which runs without a MATLAB license, wrapped in an **Apptainer image**, `container/cat12.def`) → parses the CAT12 XML report into the same `atlas__region__metric` feature space `bagpipe.db.ingest_cat12` produces (`bagpipe.app.cat12_parse`) → QC checks → inference with the registered production model, Cole-corrected using that model's own stored OOF `predictions` (no separate corrector artifact needed) → regional norm comparison (age/sex/TIV-adjusted z-scores against the SNBB population, the same approach as the maintainer's separate `normative` project, ported in-line as `bagpipe.app.normative` rather than depended on as a package) → HTML report rendered to PDF (WeasyPrint) → emailed → uploaded data deleted (default) or retained with explicit opt-in consent, in which case it's ingested into the DB via the standard Pillar 1 path.

**Why Apptainer, not Docker:** the workstation runs single-user, no root-daemon container runtime is installed, and Apptainer/Singularity is the standard for MATLAB-Compiler-Runtime-based neuroimaging tools (CAT12 standalone) on shared HPC-style setups — it needs no daemon and its `.sif` image is a single portable file. `container/cat12.def` is the build recipe; the built `.sif` lives at `paths.cat12_apptainer_image` (git-ignored, not committed — CAT12's standalone installer sits behind Neuro Debian's login wall, so the `.def` documents the install step rather than fetching it unattended). **Pinned to the exact CAT12 build that produced the training features (CAT12.9_2577, see CLAUDE.md "Machine-specific facts")** — a version drift here silently biases every public prediction relative to the model it's served against.

Stack: **FastAPI** for the web app (`bagpipe.app.api`, `bag app serve`), a lightweight SQLite-backed job queue (Huey or a simple jobs table — no Redis needed at this scale), and the existing workstation GPU for inference. "Processed within X hours, emailed" matches this perfectly — CAT12 takes tens of minutes per scan on CPU, which is fine for an async queue. *The first working version of `bagpipe.app.pipeline`/`api.py` runs synchronously in the request handler — fine for local testing of the CAT12→predict→norms chain end to end, but it will time out a real HTTP request; the queue/worker split is the next step before this is public-facing, along with the report template and email delivery below.*

Report contents: predicted age and corrected BAG with an uncertainty range; normative percentile curves (BAG and key regional volumes against the SNBB age distribution, using the model's own predicted age as the conditioning age — a fresh upload has no chronological age to condition on); hippocampal, ventricular, and total GM/WM/CSF measures; a QC summary; and a clear, prominent statement that this is a research/informational tool, not a medical device, with no diagnostic claims. (When this actually opens to the public, it's worth one conversation with your institution about consent text and liability wording — flagging it now so it's not a surprise later.)
...
**Phase 4 — Web app.** Apptainer CAT12 preprocessing image (`container/cat12.def`), FastAPI app (upload → NIfTI/DICOM → CAT12 → predict → regional norms), then queue + worker, report template, email delivery, deletion/consent logic. Deliverable: locally deployed service processing a DICOM-or-NIfTI upload end to end.

*Status (2026-08-19): kicked off, ahead of Phase 3 finishing (maintainer asked for it directly). Pipeline module (`src/bagpipe/app/pipeline.py`) wires
`to_nifti` (dcm2niix, DICOM zip/dir or passthrough for `.nii`/`.nii.gz`) → `deface` (pydeface) → `run_cat12` (Apptainer, `container/cat12.def`) → `bagpipe.app.cat12_parse.extract_features` (ported from the maintainer's private `update_tabular_cat12.py` XML parsing, adapted to a single fresh CAT12 run instead of a derivatives-tree walk) → `predict_and_score`: vectorizes onto the promoted model's exact training column space (`bagpipe.models.tabular.region_columns_for`, new), predicts, Cole-corrects using the model's own stored `predictions` rows (no separate corrector artifact — always in sync with the deployed model), and scores regional z-scores via `bagpipe.app.normative` (age/sex/TIV-adjusted OLS per region, the `normative` project's approach ported in-line). FastAPI app (`bagpipe.app.api`, `bag app serve`) exposes `POST /predict`. Synthetic tests cover CAT12 XML parsing, norm-fitting, and the full predict/score path (stubbed model+DB) — `tests/test_cat12_parse.py`, `tests/test_normative.py`, `tests/test_pipeline.py`, `tests/test_tabular.py::test_region_columns_for`. **Not yet run against real data or a real CAT12 container** — `container/cat12.def` documents the Apptainer build but the CAT12 standalone MCR install step is a manual, license-gated download, not scripted. Not yet done: job queue, report template/PDF, email delivery, consent/deletion logic, and verifying the whole chain against a real uploaded scan once the `.sif` is built.*
